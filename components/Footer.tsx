...
const Footer: React.FC = () => {
  ...
  // Note: For the actual production site, replace this link with the specific 
  // destination from your QR code to ensure the dynamic generator matches your account.
  const wechatData = "https://u.wechat.com/AFWIA_OFFICIAL"; 
    ...
    <footer className="bg-black text-white py-24 px-6 md:px-12 border-t border-black relative">
      ...
      {showWeChat && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          />
          <div className="relative bg-white pt-14 pb-14 px-8 md:px-14 max-w-[400px] w-full animate-in fade-in zoom-in duration-300 shadow-2xl rounded-2xl">
            <button 
              onClick={() => setShowWeChat(false)}
              className="absolute top-6 right-6 text-black/10 hover:text-black transition-colors p-1"
            >
              <X size={24} />
            </button>
            ...
            <div className="flex flex-col items-center">
              ...
              {/* High Fidelity QR Implementation matching provided image */}
              <div className="relative w-full aspect-square bg-white flex items-center justify-center mb-10 p-2 border border-gray-50 rounded-lg">
                <img 
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=600x600&data=${encodeURIComponent(wechatData)}&ecc=H&margin=2`} 
                  alt="WeChat QR Code" 
                  className="w-full h-full block object-contain"
                />
                
                {/* Center Logo: Black Rounded Square + High Fidelity WeChat Bubble Icon */}
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="w-[19%] h-[19%] bg-white p-[3px] rounded-[24%] shadow-sm">
                    <div className="w-full h-full bg-black flex items-center justify-center rounded-[22%]">
                      {/* High-Fidelity WeChat SVG with eyes as seen in user reference */}
